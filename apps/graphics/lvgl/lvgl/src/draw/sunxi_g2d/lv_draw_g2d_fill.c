                      ...
                      const lv_area_t * coords)
{
    ...
    if(draw_buf == NULL || draw_buf->data == NULL ||
       blend_area.x1 < 0 || blend_area.y1 < 0 ||
       blend_area.x2 >= draw_buf->header.w || blend_area.y2 >= draw_buf->header.h)
        return;

    uint8_t * dest_buf = draw_buf->data;
    uint32_t dest_stride = draw_buf->header.stride;
    lv_color_format_t dest_cf = draw_buf->header.cf;
    _g2d_fill(dest_buf, &blend_area, dest_stride, dest_cf, dsc);
}
                      ...
                      lv_color_format_t dest_cf, const lv_draw_fill_dsc_t * dsc)
{
    ...
    if(px_size == 0)
        return;
    unsigned long vaddr_start = (unsigned long)dest_buf +
                                dest_stride * dest_area->y1 +
                                px_size * dest_area->x1;
    size_t cache_size = (size_t)(dest_h - 1) * dest_stride +
                        (size_t)dest_w * px_size;
    hal_dcache_clean_invalidate(vaddr_start, cache_size);
