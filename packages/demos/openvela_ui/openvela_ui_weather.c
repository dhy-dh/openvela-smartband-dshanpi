                                ...
                                const char *administrative_area)
{
    ...
    pthread_mutex_lock(&g_weather.lock);
    /* Compare the previous query key, not response display fields.  The
     * proxy expands response names (for example adm2 + adm1), which must not
     * make a periodic refresh look like a different city. */
    same_location = strcmp(g_weather.request.location_id,
                           location_id ? location_id : "") == 0 &&
                    strcmp(g_weather.request.location, location) == 0 &&
                    strcmp(g_weather.request.administrative_area,
                           administrative_area ? administrative_area : "") == 0;
